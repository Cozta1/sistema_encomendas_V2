    ...
    function ClienteListPage() {
      ...
      const fetchData = useCallback(async (page = 1, currentSearch = searchTerm) => {
        ...
        // Adicionar ordenação se necessário: params.append('ordering', 'nome');
        ...
        try {
          ...
          const pageSize = response.data.results?.length > 0 ? response.data.results.length : 20; // Ajuste page size se necessário
          setTotalPages(Math.ceil((response.data.count || 0) / pageSize));
          setCurrentPage(page);

          // Tenta buscar o nome da equipe (pode vir do primeiro cliente ou fazer outra chamada)
          if (response.data.results?.length > 0 && response.data.results[0].equipe_nome) {
               setEquipeNome(response.data.results[0].equipe_nome);
          } else if (clientes.length === 0) {
               // Se não houver clientes, busca o nome da equipe separadamente (melhor seria ter um endpoint /api/equipes/{id}/)
               try {
                   const equipeRes = await api.get(`/equipes/${equipeId}/`); // Assume endpoint de detalhe da equipe
                   setEquipeNome(equipeRes.data.nome);
      ...
      }, [equipeId, searchTerm, setSearchParams, clientes.length]); // Inclui clientes.length para tentar buscar nome da equipe
      ...
      }, [fetchData, searchParams]); // Re-executa se fetchData ou searchParams mudarem
        ...
        <div>
                    ...
                    <table className="table table-hover mb-0">
                      <thead>
                        <tr>
                          ...
                          <th>Bairro</th>
                          <th>Telefone</th>
                          {/* <th>Encomendas (Equipe)</th> */} {/* Remover contagem daqui por enquanto */}
                          <th>Ações</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          <tr key={cliente.id ?? cliente.pk}>
                            ...
                            <td>
                              <div className="btn-group btn-group-sm">
                                ...
                                <Link to={`/clientes/${cliente.id ?? cliente.pk}/editar/equipe/${equipeId}`} className="btn btn-outline-secondary" title="Editar Cliente"> {/* Rota a ser criada */}
                                  <i className="bi bi-pencil"></i>
                                </Link>
                                {/* Botão Excluir (implementar lógica depois) */}
                                <button onClick={() => {/* TODO */}} className="btn btn-outline-danger" title="Excluir Cliente" disabled>
          ...
          <div className="mt-4">
             <Link to={`/dashboard/${equipeId}`} className="btn btn-secondary">
                 <i className="bi bi-arrow-left me-2"></i>Voltar ao Dashboard da Equipe
             </Link>
          </div>
        </div>
      );
